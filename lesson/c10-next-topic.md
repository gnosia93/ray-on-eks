...
3. 전체 파이프라인 시나리오 (워크숍 피날레)
워크숍의 마지막을 장식할 만한 전체 흐름은 다음과 같습니다.
Airflow 시작: S3에 새로운 데이터가 들어오면 감지(Sensor).
Ray Task: Airflow가 Ray에 명령을 내려 C8(대규모 전처리) 실행.
Check: 전처리가 완료되어 S3에 Parquet 파일이 생겼는지 확인.
Hand-over: Airflow가 Slurm 클러스터에 SSH나 API로 신호를 보내 학습(PyTorch) 시작.
